...
(function (global) {
  ...
  function setupLanguages(l) {
    ...
    $.each(l, function (i, lang) {
      languages[lang.tag] = lang;
    });
    ...
    if ( defaultLanguage !== null
      ...
      return activateLanguage(defaultLanguage);
    }

    return  activateLanguage(languages[Object.keys(languages)[0]].tag);
  }

  // if we click on a language tab, activate that language
  $(function() {
    var langSelector = $('.lang-selector')[0];
    var els = $('pre, blockquote, section');
    var validEls = [];

    $.each(els, function(k, el) {
     if(  !$(el).prev().is('pre')
       && !$(el).prev().is('blockquote')
       && !$(el).prev().is('section')
       && !$(el).prev().is('aside')
     ) {
       validEls.push(el)
    ...
    $.each(validEls, function(k, el) {
      $(el).prev().addClass('clear');

      $(el)
        .before(
          $(langSelector).clone()
            .css('float', 'right').css('position', 'relative')
         );
    });
