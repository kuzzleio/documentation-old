...
A subscription opens a permanent pipe between the Kuzzle SDK client and the Kuzzle Backend. Whenever a real-time message or a modified document matches a subscription filter, a push notification is sent by Kuzzle Backend to the Kuzzle SDK client (for instance, see the [Collection.subscribe]({{ site_base_path }}sdk-reference/collection/subscribe) method).
