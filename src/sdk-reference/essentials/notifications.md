...
This means, that when you want to subscribe you must provide a set of filter definitions, using [real-time DSL]({{ site_base_path }}kuzzle-dsl), that tell Kuzzle Backend what documents should trigger a notification. Then, any time a document matches the defined filters, Kuzzle Backend will send a push notification to the subscriber.
