...
| ``defaultIndex`` | string | Kuzzle Backend's default index to use |
| ``headers`` | JSON object | Common headers for all sent documents. |
| ``host`` | string | Target Kuzzle Backend host name/address |
...
| ``port`` | integer | Kuzzle Backend network port |
...
| ``sslConnection`` | boolean | Connect to Kuzzle Backend using SSL |
...
* multiple methods allow passing specific ``volatile`` data. These ``volatile`` data will be merged with the global Kuzzle object ``volatile`` object when sending the request, with the request specific ``volatile`` taking priority over the global ones.
